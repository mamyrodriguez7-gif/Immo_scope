...
import numpy as np  # Utilisé pour certaines opérations
...
class MapVisualizer:
    ...
    def __init__(self):
        ...
        # Ajouter une colonne médiane (utile dans certains cas)
        self.df['prix_m2_median'] = self.df['prix_m2'].median()
    ...
    # -------------------------------------------------------------
    # CARTE 1 : Prix moyen au m² par département (Choroplèthe)
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # CARTE 2 : Scatter Mapbox (transactions individuelles)
    # -------------------------------------------------------------
    def create_scatter_map(self, df_filtered: pd.DataFrame):
        """
        Affiche une carte détaillée des transactions individuelles avec nom de commune.
        """

        # Échantillonner pour éviter surcharge (max 2000 points)
